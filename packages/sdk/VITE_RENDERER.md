# Vite renderer integration (`@seedprotocol/sdk/vite`)
...
- **Node polyfills** (`vite-plugin-node-polyfills`) with browser-safe defaults (optional via `includeNodePolyfills: false`).
...
2. Align **Vite 8** / **plugin-react 6** / **vite-plugin-node-polyfills 0.25+** with your app.
...
|--------|---------|-------------|
| `includeNodePolyfills` | `true` | Add `vite-plugin-node-polyfills` with SDK defaults |
