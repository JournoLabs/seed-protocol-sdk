 ...
 */
export async function findPropertyNameBySchemaFileId(
  schemaName: string,
  modelName: string,
  propertySchemaFileId: string,
): Promise<string | undefined> {
  // 1. Try schema file lookup first
    ...
    try {
      ...
      if (model?.properties) {
        for (const [propName, prop] of Object.entries(model.properties)) {
          if (prop.id === propertySchemaFileId) {
            return propName
          }
        }
      }
    } catch (err) {
      logger('Schema file lookup failed, trying DB fallback:', err)
    }
  }

  // 2. Fallback: look up in database (properties table by schemaFileId and modelId)
  const db = BaseDb.getAppDb()
  if (!db) {
    return undefined
  }
  ...
  try {
    ...
    if (schemaRecord.length === 0) return undefined
    ...
    const modelRecords = await db
      ...
      .where(
        and(
          eq(modelSchemas.schemaId, schemaRecord[0].id),
    ...
    const propertyRecords = await db
      ...
      .where(
        and(
          eq(propertiesTable.modelId, modelRecords[0].modelId!),
          eq(propertiesTable.schemaFileId, propertySchemaFileId),
        ),
      )
      .limit(1)

    if (propertyRecords.length > 0) {
      return propertyRecords[0].name
    }
  } catch (err) {
    logger('DB fallback for findPropertyNameBySchemaFileId failed:', err)
  }
...
export async function renameModelProperty(
  ...
  setTimeout(() => {
    loadSchemaFromFile(newFilePath).catch((err) => {
      logger('Deferred loadSchemaFromFile after rename failed:', err)
    })
  }, 100)
