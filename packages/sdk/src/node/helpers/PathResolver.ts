...
class PathResolver extends BasePathResolver {
  ...
  private isInSdkRepo(): boolean {
    try {
      ...
      // Check when running from packages/sdk (SDK package root)
      if (fs.existsSync(path.join(currentDir, 'src', 'node')) &&
          fs.existsSync(path.join(currentDir, 'package.json')) &&
          JSON.parse(fs.readFileSync(path.join(currentDir, 'package.json'), 'utf8')).name === '@seedprotocol/sdk') {
        return true
      }
      // Check when running from monorepo root
      if (fs.existsSync(path.join(currentDir, 'packages', 'sdk', 'src', 'node')) &&
          fs.existsSync(path.join(currentDir, 'packages', 'sdk', 'package.json')) &&
          JSON.parse(fs.readFileSync(path.join(currentDir, 'packages', 'sdk', 'package.json'), 'utf8')).name === '@seedprotocol/sdk') {
        return true
      }
      return false
    } catch {
  ...
  getSdkRootDir(): string {
    ...
    if (env === 'sdk-dev') {
      // When running from monorepo root, SDK root is packages/sdk
      if (fs.existsSync(path.join(rootWithNodeModules, 'packages', 'sdk', 'package.json'))) {
        return path.join(rootWithNodeModules, 'packages', 'sdk')
      }
      // When running from packages/sdk directly
      return rootWithNodeModules
    }
