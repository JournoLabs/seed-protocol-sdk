  ...
  const _resolveRelatedValue = async () => {
    ...
    if (relationValueData) {
      ...
      if (
        ...
        const imagePath = BaseFileManager.getFilesPath('images', refResolvedValue)
        const fileExists = await BaseFileManager.pathExists(imagePath)
        if (fileExists) {
          const file = await BaseFileManager.readFile(imagePath)
